...
Catalog *read_random_catalog(char *fname){
  external_random_cat = 1;
  np_t sum_w = 0, sum_w2 = 0;
  Catalog *cat = read_catalog(fname,&sum_w,&sum_w2);
  sum_w_global_random_catalog = sum_w;
  sum_w2_global_random_catalog = sum_w2;
  return cat;
}

void free_random_catalog(){
  if(external_random_cat){
    free_Catalog(global_random_catalog);
    external_random_cat = 0;
  }
}
