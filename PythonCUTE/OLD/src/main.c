    ...
    int nbins)
{
  ...
  *R1R2=my_calloc(nbins,sizeof(histo_t));
  ...
  if(external_random_cat){
    cat_ran = global_random_catalog;
    *sum_wr = sum_w_global_random_catalog;
    *sum_wr2 = sum_w2_global_random_catalog;
  } else {
...
void run_full_corr_bf(void)
{
  ...
  if(!external_random_cat) free_Catalog(cat_ran);
  if(use_two_catalogs) {
...
int runCUTE(Catalog *random_catalog){
  int argc = 1;
  char **argv = NULL;
  mpi_init(&argc,&argv);
  ...
  if(random_catalog != NULL){
    ...
    print_info("External random catalog has np = %d [w = %d w2 = %d]\n", global_random_catalog->np, 
        (int)sum_w_global_random_catalog, (int)sum_w2_global_random_catalog);
  }
