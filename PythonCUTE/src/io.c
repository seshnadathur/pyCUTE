  ...
  if(NodeThis==0) {
    ...
    if(corr_type==0) {
      for(ii=0;ii<nb_dz;ii++) {
        ...
        set_result(result_global, ii, dz, corr, (double)D1D2[ii], (double)D1R2[ii], (double)R1D2[ii], (double)R1R2[ii]);
#endif
      }
    }
    else if(corr_type==1) {
...
#endif //_WITH_WEIGHTS
#ifdef _CUTE_AS_PYTHON_MODULE
        set_result(result_global, ii, th, corr, (double)D1D2[ii], (double)D1R2[ii], (double)R1D2[ii], (double)R1R2[ii]);
#endif
      }
    }
    else if(corr_type==2) {
...
#endif //_WITH_WEIGHTS
#ifdef _CUTE_AS_PYTHON_MODULE
        set_result(result_global, ii, rr, corr, (double)D1D2[ii], (double)D1R2[ii], (double)R1D2[ii], (double)R1R2[ii]);
#endif
      }
    }
    else if(corr_type==3) {
...
int verify_parameters(){
  ...
  if(global_binner.logbin<0) {
    fprintf(stderr,"CUTE: logarithmic binning option not provided\n");
    error += 1;
  }
  if(global_binner.dim1_nbin<=0) {
    fprintf(stderr,"CUTE: wrong #bins for dim1 %d\n",global_binner.dim1_nbin);
    error += 1;
  }
  if(global_binner.dim2_nbin<=0) {
    fprintf(stderr,"CUTE: wrong #bins for dim2 %d\n",global_binner.dim2_nbin);
    error += 1;
  }
  if(global_binner.dim3_nbin<=0) {
    fprintf(stderr,"CUTE: wrong #bins for dim3 %d\n",global_binner.dim3_nbin);
    error += 1;
  }
  if(global_binner.dim1_max<=0) {
    fprintf(stderr,"CUTE: wrong dim1_max %lf\n",global_binner.dim1_max);
    error += 1;
  }
  if(global_binner.logbin) {
    if((global_binner.dim1_min<=0) || (global_binner.dim1_min>=global_binner.dim1_max)) {
      fprintf(stderr,"CUTE: wrong lower limit for logarithmic binning %lf\n",global_binner.dim1_min);
      error += 1;
    }
  }
  if(global_binner.dim2_max<=0) {
    fprintf(stderr,"CUTE: wrong dim2_max %lf\n",global_binner.dim2_max);
    error += 1;
  }
  if((global_binner.dim3_max<=0)||(global_binner.dim3_min<0)||
      (global_binner.dim3_max<=global_binner.dim3_min)) {
    fprintf(stderr,"CUTE: wrong boundaries for dim3 (%lf , %lf)\n",
        global_binner.dim3_min,global_binner.dim3_max);
    error += 1;
  }

  logbin=global_binner.logbin;
  if(logbin)
    n_logint=global_binner.dim1_nbin/log10(global_binner.dim1_max/global_binner.dim1_min);
  if(corr_type==0) {
    nb_dz=global_binner.dim1_nbin;
    i_dz_max=1./global_binner.dim1_max;
  }
  else  if(corr_type==1) {
    nb_theta=global_binner.dim1_nbin;
    i_theta_max=1./(DTORAD*global_binner.dim1_max);
    log_th_max=log10(DTORAD*global_binner.dim1_max);
  }
  else if(corr_type==2) {
...
void set_data_filename(char *s){
  sprintf(fnameData1,"%s",s);
}
void set_data_filename_2(char *s){
  sprintf(fnameData2,"%s",s);
}
void set_random_filename(char *s){
  sprintf(fnameRandom1,"%s",s);
}
void set_random_filename_2(char *s){
  sprintf(fnameRandom2,"%s",s);
}
void set_RR_filename(char *s){
  sprintf(fnameRR,"%s",s);
}
...
void set_dim1_min_logbin(double x){
  global_binner.dim1_min=x;
}
