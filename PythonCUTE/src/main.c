...
int runCUTE(Catalog *random_catalog, Result *res){
...
int main(int argc,char **argv){
  ...
  if(random_catalog != NULL){
    ...
    print_info("Using external random catalog with np = %d  w = %d  w2 = %d\n", global_random_catalog->np, 
        (int)random_catalog->sum_w, (int)random_catalog->sum_w2);
  } else {
    global_random_catalog = NULL;
  }
  result_global = res;

  if(corr_type > 2){
    print_info("Returning result to Python not implemnted for corr_type > 2. Will just output results to file!\n");
    result_global = NULL;
  }
